...
The Cosmos of SSE events lives in `/api/projects/:id/generation-stream`.
 That route creates a `TransformStream` that replays `project_generation_events` since the last seen ID, keeps the connection alive with a `: ping` every 20 seconds, and wires live listeners so the queue consumer’s persisted events plus live-only transient events can both reach the frontend immediately. Event payloads now include `thinking` alongside the durable batch events (`batch_start`, `activity`, `batch_complete`, `checkpoint`, `pipeline_complete`, `pipeline_failed`), which the frontend decodes to drive the live status feed, review panel, and progress dots. Persisted events still replay from D1 on reconnect; `thinking` events are intentionally ephemeral so the audit trail stays clean.
...
The backend emits durable `batch_start`, `activity`, `batch_complete`, `checkpoint`, `pipeline_complete`, and `pipeline_failed` SSE events that the frontend uses to animate the generation heading, the historical activity log, progress dots, ETA counter, and review gate. Provider reasoning chunks are streamed separately as transient `thinking` events: they are published live to the open SSE connection, never written to `project_generation_events`, and disappear on reconnect while the persisted history replays from D1. This split lets Scrimble stream live model thinking without polluting the durable audit trail.
...
- A scrollable activity log (max-height 280px) streams persisted events (`activity`, `batch_complete`, `checkpoint`, `pipeline_failed`) with icons (🔍, 📦, ⚠️, 🏗️, ✅, 📝), timestamps (HH:MM:SS JetBrains Mono), and auto-scroll behavior. Each event is durable, so reconnecting clients replay history via `Last-Event-ID` while the live line resumes with fresh transient thinking.
